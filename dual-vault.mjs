// dual-vault.mjs
// Exported function initDualVault(container = document.body)
// By: converted from original inline <script>

const CSS_ID = 'dual-vault-styles-v10';
const MARKUP_ID = 'dual-vault-markup-v10';
...
function createMarkup() {
  // returns the main HTML markup as string (no inline onclicks)
  return `
  <div id="${MARKUP_ID}">
    ...
    <div class="container">
        ...
        <div class="fusion-card compact" id="mainCard">
            ...
            <div class="modules-area" id="modulesArea">
                <div class="modules-inner">
                    <input class="cyber-input" id="inputUser" placeholder="Identifique-se para calibração..." autocomplete="off">
                    
                    <div class="ascii-block" id="asciiDisplay" style="display:none;"></div>
                    <div class="action-row" id="asciiActions" style="display:none;">
                         <button class="btn-fusion" id="btnCopyActivation">COPIAR DNA</button>
                         <button class="btn-fusion" id="btnExportActivation">EXPORTAR PNG</button>
                    </div>
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <button class="icon-btn" id="btnPasteMagic" title="Clipboard Paste"><i data-lucide="clipboard-copy"></i></button>
                    </div>
                    ...
                    <div class="action-row">
                        ...
                        <button class="btn-fusion accent" id="btnInjectManual">SALVAR NO VAULT</button>
                        <input type="file" id="importFile" hidden>
                        <button class="btn-fusion" id="btnImportFile" title="Importar Backup"><i data-lucide="upload"></i></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="filter-bar" id="filterBar">
            ...
            <div class="filter-pill" data-filter="CODE">CODE</div>
            <div class="filter-pill" data-filter="DOCS">DOCS</div>
            <div class="filter-pill" data-filter="CLIPBOARD">CLIPBOARD</div>
        </div>
...
function defaultCss() {
  return `@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;600;800&family=JetBrains+Mono:wght@400;700&display=swap');
...
// hashing and 3-6-9 logic (kept from original)
...
// The main exported initializer
export default function initDualVault(container = document.body) {
  // inject CSS
  injectStyles(defaultCss());
  ...
  // cache DOM nodes
  const root = document.getElementById(MARKUP_ID);
  const mainCard = document.getElementById('mainCard');
  const cardHeader = document.getElementById('cardHeader');
  const inputUser = document.getElementById('inputUser');
  const userNameDisplay = document.getElementById('userNameDisplay');
  const userVibeBadge = document.getElementById('userVibeBadge');
  const avatarTarget = document.getElementById('avatarTarget');
  const asciiDisplay = document.getElementById('asciiDisplay');
  const asciiActions = document.getElementById('asciiActions');
  const btnCopyActivation = document.getElementById('btnCopyActivation');
  const btnExportActivation = document.getElementById('btnExportActivation');
  const btnPasteMagic = document.getElementById('btnPasteMagic');
  const injectorContent = document.getElementById('injectorContent');
  const injectorGroup = document.getElementById('injectorGroup');
  ...
  const mainChevron = document.getElementById('mainChevron');
  ...
  function renderIdentity() {
    const root369 = reduce369(currentUser);
    userNameDisplay.textContent = (currentUser || 'CONVIDADO').toUpperCase();
    userVibeBadge.textContent = `v:${root369 === '--' ? '--' : root369}`;
    avatarTarget.innerHTML = generateAvatar(currentUser);

    if ([3,6,9].includes(root369)) {
      mainCard.classList.add('vibe-gold');
      userVibeBadge.classList.add('gold');
    } else {
      mainCard.classList.remove('vibe-gold');
      userVibeBadge.classList.remove('gold');
    }
  ...
  function updateIdentityFromInput() {
  ...
  function toggleMainCard() {
    ...
    mainCard.classList.toggle('compact');
    mainChevron.style.transform = mainCard.classList.contains('expanded') ? 'rotate(180deg)' : 'rotate(0deg)';
    if (mainCard.classList.contains('expanded') && !inputUser.value) inputUser.focus();
  }

  function generateAscii() {
    if(!currentUser) { asciiDisplay.style.display='none'; asciiActions.style.display='none'; return; }
    ...
    asciiDisplay.textContent = txt;
    asciiDisplay.style.display = 'block';
    asciiActions.style.display = 'flex';
  }
  ...
  function renderVault() {
    ...
    if(filtered.length === 0) {
      vaultGrid.innerHTML = `<div style="text-align:center; padding:20px; color:rgba(255,255,255,0.3); font-family:var(--font-code);">Nenhum dado no fluxo.</div>`;
      lucide && lucide.createIcons && lucide.createIcons();
      return;
    }
    ...
    filtered.forEach(item => {
      ...
      el.id = `card-${item.id}`;
      if((item.content||'').includes('369') || (item.group||'').toUpperCase() === 'GOLD') el.style.borderColor = 'rgba(255,210,80,0.4)';
      const dateStr = new Date(item.date).toLocaleDateString('pt-BR');

      el.innerHTML = `
        <div class="vc-header" data-id="${item.id}">
          <div style="flex:1; overflow:hidden;">
            <div class="vc-title">${escapeHtml(item.title)}</div>
            <div class="vc-meta">
              ${item.group ? `<span class="vc-tag">${escapeHtml(item.group)}</span>` : ''}
              <span>${escapeHtml(dateStr)}</span>
            </div>
          </div>
          <div class="vc-toolbar" data-id="${item.id}">
            <button class="icon-btn btn-preview" data-id="${item.id}" title="Preview"><i data-lucide="play"></i></button>
            <button class="icon-btn btn-copy" data-id="${item.id}" title="Copy"><i data-lucide="copy"></i></button>
            <button class="icon-btn destruct btn-delete" data-id="${item.id}" title="Delete"><i data-lucide="trash-2"></i></button>
          </div>
        </div>
        <div class="vc-content" id="content-${item.id}">
          <textarea class="cyber-input cyber-area" readonly>${escapeHtml(item.content)}</textarea>
    ...
    // attach listeners for each card's controls
    vaultGrid.querySelectorAll('.vc-header').forEach(h => {
      const id = h.getAttribute('data-id');
      h.addEventListener('click', () => toggleItem(id));
    });
    vaultGrid.querySelectorAll('.btn-preview').forEach(b => {
      const id = b.getAttribute('data-id');
      b.addEventListener('click', (ev) => { ev.stopPropagation(); renderFrame(id); });
    });
    vaultGrid.querySelectorAll('.btn-copy').forEach(b => {
      const id = b.getAttribute('data-id');
      b.addEventListener('click', (ev) => { ev.stopPropagation(); copyItem(id); });
    });
    vaultGrid.querySelectorAll('.btn-delete').forEach(b => {
      const id = b.getAttribute('data-id');
      b.addEventListener('click', (ev) => { ev.stopPropagation(); deleteItem(id); });
    });
  ...
  function injectManual() {
    const content = injectorContent.value;
    const group = (injectorGroup.value || '').toUpperCase();
    if(!content.trim()) return;
    const title = (content.split('\n')[0] || 'Untitled Node').substring(0,40);
    vaultItems.unshift({
      id: uid(),
    ...
    saveVault();
    injectorContent.value = '';
    // close main card for UX parity
    if (mainCard.classList.contains('expanded')) toggleMainCard();
  }

  async function pasteMagic() {
    try {
      const text = await navigator.clipboard.readText();
      injectorContent.value = text;
  ...
  function renderFrame(id) {
    ...
    box.innerHTML = `
      <div style="display:flex; justify-content:flex-end; gap:8px; margin-bottom:8px; margin-top:8px;">
          <button class="vc-tag" data-action="cinema" data-id="${id}" style="cursor:pointer; border:none;">⛶ CINEMA MODE</button>
          <button class="vc-tag" data-action="close" data-id="${id}" style="cursor:pointer; border:none; color:#ef4444;">FECHAR</button>
      </div>
    ...
    box.querySelectorAll('[data-action]').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ...
        if(action === 'cinema') launchCinema(id);
        if(action === 'close') box.style.display = 'none';
      });
    });
  }
  ...
  function launchCinema(id) {
    ...
    btn.style.cssText = 'background:#ef4444; color:white; border:none; padding:8px 16px; border-radius:4px; font-weight:bold; cursor:pointer;';
    btn.textContent = 'SAIR DO MODO CINEMA';
  ...
  function copyItem(id) {
    const item = vaultItems.find(i => i.id === id);
    if(!item) return;
    navigator.clipboard.writeText(item.content).catch(()=>{/* ignore */});
  }

  function filterVault(filter) {
    currentFilter = filter;
    const pills = filterBar.querySelectorAll('.filter-pill');
    pills.forEach(p => p.classList.toggle('active', p.getAttribute('data-filter') === filter));
    renderVault();
  }
    ...
    r.onload = e => {
      try {
        ...
        if(Array.isArray(data)) {
          data.forEach(d => { if(!d.id) d.id = uid(); vaultItems.unshift(d); });
          saveVault();
  ...
  function copyActivation() {
    const txt = asciiDisplay.textContent || '';
    navigator.clipboard.writeText(txt).catch(()=>{/* ignore */});
  }

  function exportActivation() {
    const target = asciiDisplay;
    if (typeof html2canvas === 'undefined') { alert('html2canvas não encontrado.'); return; }
    html2canvas(target, { backgroundColor: '#000' }).then(canvas => {
      const a = document.createElement('a');
      a.download = 'dual-identity.png';
      a.href = canvas.toDataURL();
      a.click();
    }).catch(()=> alert('Erro ao gerar PNG.'));
  }

  // CLOCK
  setInterval(() => {
    clockDisplay.textContent = new Date().toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'});
  }, 1000);

  // BIND UI EVENTS
  cardHeader.addEventListener('click', toggleMainCard);
  cardHeader.addEventListener('keydown', (e) => { if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleMainCard(); }});
  inputUser.addEventListener('input', updateIdentityFromInput);
  btnCopyActivation.addEventListener('click', copyActivation);
  btnExportActivation.addEventListener('click', exportActivation);
  btnPasteMagic.addEventListener('click', pasteMagic);
  btnInjectManual.addEventListener('click', injectManual);
  importFile.addEventListener('change', handleImportFile);
  btnImportFile.addEventListener('click', () => importFile.click());
  filterBar.querySelectorAll('.filter-pill').forEach(p => {
    p.addEventListener('click', () => filterVault(p.getAttribute('data-filter')));
  });

  // Boot: restore state and initial render
  if (currentUser) {
    inputUser.value = currentUser;
    renderIdentity();
  } else {
    // open if guest (same behaviour as original)
    if(!mainCard.classList.contains('expanded')) toggleMainCard();
  }
  renderVault();

  // expose minimal API on the wrapper element for optional external control
  root.dualVault = {
    saveVault,
    renderVault,
    getItems: () => vaultItems,
    addItem: (item) => { item.id = item.id || uid(); item.date = item.date || nowIso(); vaultItems.unshift(item); saveVault(); }
  };

  // ensure icons drawn
  lucide && lucide.createIcons && lucide.createIcons();

  // return the root node (handy)
  return root;
}